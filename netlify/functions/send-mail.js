...
exports.handler = async (event) => {
    ...
    if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
        ...
        return { statusCode: 500, body: "Sunucu yapılandırma hatası (Eksik parametre)." };
    }
    ...
    if (type === 'reset') {
        mailOptions = {
            ...
            to: email,
            subject: 'HediyeEşle - Şifre Sıfırlama İsteyi',
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
                    <h2 style="color: #6366f1; text-align: center;">Şifre Sıfırlama</h2>
                    <p>Merhaba,</p>
                    <p>HediyeEşle hesabınız için şifre sıfırlama talebinde bulundunuz. Şifrenizi sıfırlamak için aşağıdaki butona tıklayabilirsiniz:</p>
                    ...
                    <p style="font-size: 12px; color: #999; text-align: center;">HediyeEşle - Mutlu Anların Paylaşım Noktası</p>
                </div>
            `,
        ...
        mailOptions = {
            ...
            replyTo: email,
            subject: `HediyeEşle İletişim: ${name}`,
            text: `Ad: ${name}\nEmail: ${email}\n\nMesaj:\n${message}`,
        };
    }
    ...
    try {
        ...
        return { statusCode: 200, body: "Mesaj başarıyla gönderildi." };
    } catch (err) {
        console.error("E-posta Gönderim Hatası:", err);
        return {
            statusCode: 500,
            body: `Gönderim hatası: ${err.message}`
        };
    }
};
