        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {[
              ...
              { t: 'Admin Paneli', p: '/admin' }
            ].map((link, i) => (
              <Link key={i} to={link.p} style={{ color: 'rgba(255, 255, 255, 0.7)', textDecoration: 'none', fontSize: '0.95rem', transition: 'var(--transition)' }}>{link.t}</Link>
            ))}
          </div>
        </div>
